// ============================================================
// STUDYSHIFT — app.js  (Gemini Edition)
// AI powered by Google Gemini 2.0 Flash (free tier)
// API key lives ONLY in Netlify env vars / .env — never here
// ============================================================
...
function loadState() {
  try {
    const r = localStorage.getItem("ss_v4");
...
function save() {
  try {
    ...
    localStorage.setItem("ss_v4", JSON.stringify(d));
  } catch (e) {}
}
...
function refreshUI() {
  const u = S.user;
  const c = CITIES[u.city] || CITIES.lahore;
  setText("sb-av", u.name[0].toUpperCase());
  setText("sb-uname", u.name);
  setText("sb-ucity", `${c.name} · Group ${u.group}`);
  setText("sched-sub", `${c.name} · Group ${u.group}`);
  const h = new Date().getHours();
  const g =
    h < 12 ? "Good morning" : h < 17 ? "Good afternoon" : "Good evening";
  setText("greeting", `${g}, ${u.name}`);
  setText(
    "today-dt",
  ...
  setText("tip-body", TIPS[Math.floor(Math.random() * TIPS.length)]);
  // AI always available via proxy — show green
  const dot = document.getElementById("sb-ai-dot");
  const txt = document.getElementById("sb-ai-txt");
  if (dot) dot.className = "bi bi-circle-fill sb-ai-dot";
  if (txt) txt.textContent = "Gemini AI Ready";
  // Update AI bar subtitle
  setText(
    "ai-bar-sub",
    "Click to fetch today's real-time load shedding schedule via AI",
  );
}
...
function renderTimeline(pat) {
  ...
  if (pills) {
    pills.innerHTML = getScheduleBlocks(pat)
      .map(
        (b) =>
          `<span class="tl-pill ${b.type}">
        <i class="bi ${b.type === "power" ? "bi-lightning-charge-fill" : "bi-plug-fill"}"></i>
...
function renderTodayPlan(pat) {
  ...
  el.innerHTML = tasks.length
    ? tasks.map((t) => taskHTML(t, isConflict(t, pat))).join("")
    : emptyState("bi-calendar-x", "No tasks yet — add your first task above.");
}
...
function isConflict(t, pat) {
  ...
  const eh = Math.min(Math.ceil(sh + t.duration / 60), 24);
...
function renderSched() {
  ...
  for (let d = 0; d < 7; d++) {
    ...
    card.innerHTML = `
      <div class="day-nm">${DAYS[dt.getDay()]}</div>
...
function renderTasks() {
  ...
  el.innerHTML = tasks.length
    ? tasks.map((t) => taskHTML(t, isConflict(t, pat))).join("")
    : emptyState("bi-inbox", "No tasks here yet.");
}
function flt(f, btn) {
  S.taskFilter = f;
  document
...
function openTaskModal(id = null) {
  S.editId = id;
  setText("modal-ttl", id ? "Edit Task" : "Add Task");
  if (id) {
    const t = S.tasks.find((t) => t.id === id);
    if (!t) return;
...
// GEMINI API
// ══════════════════════════════════════════
// All calls go to /api/gemini (proxy route)
// Local:   server.py reads key from .env
// Netlify: gemini-proxy.js reads GEMINI_API_KEY env var
// The key is NEVER in any JS/HTML file.
...
Return exactly 24 hourly values (0=outage, 1=power available).`;
  ...
  try {
    ...
    setTimeout(() => {
      ...
      btn.style.background = "";
      btn.disabled = false;
      setAIBarSub("Click to fetch real-time schedule via Gemini AI");
    }, 5000);
  } finally {
    S.busy = false;
    btn.disabled = false;
...
async function sendMsg() {
  ...
  // Build conversation history as text for Gemini
  const historyTxt = S.chatHistory
    .slice(-6)
    .map((m) => `${m.role === "user" ? "Student" : "Assistant"}: ${m.content}`)
    .join("\n");

  const systemCtx = `You are StudyShift AI — a helpful assistant for Pakistani students dealing with load shedding.
Be friendly, concise, and specific to Pakistan's electricity situation.

Current user info:
- Name: ${S.user.name} | City: ${c.name} (${c.utility}) | Group: ${S.user.group}
- Power today: ${st.power}h on (${pwrBlks}) | Outage: ${st.outage}h (${outBlks})
- Tasks: ${
    S.tasks
      .filter((t) => !t.done)
      .map((t) => t.name)
...
${historyTxt ? "Conversation so far:\n" + historyTxt + "\n" : ""}Keep responses under 200 words. Format clearly.`;

  try {
    const reply = await callGemini(msg, systemCtx);
    thinking.remove();
    ...
    addMsg(
      "assistant",
      `Sorry, something went wrong: <em>${esc(e.message)}</em><br><br>Make sure <strong>GEMINI_API_KEY</strong> is set in Netlify → Site configuration → Environment variables.`,
    );
  } finally {
    S.chatBusy = false;
...
function addThinking() {
  ...
  d.innerHTML = `<div class="cmsg-av"><i class="bi bi-robot"></i></div>
    <div class="cmsg-bbl"><div class="thinking-dots"><span></span><span></span><span></span></div></div>`;
  msgs.appendChild(d);
  msgs.scrollTop = msgs.scrollHeight;
  return d;
}
